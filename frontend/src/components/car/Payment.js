...
const Payment = (props) => {
    ...
    const { stripeApiKey, clientSecret, order } = useSelector(state => state.order);
    const { shoppingCarId } = useSelector((state) => state.car);
        ...
        try {
            ...
            else
            {
                if (result.paymentIntent.status === 'succeeded')
                {
                    const params = {
                        orderId: order.id,
                        shoppingCarMasterId: shoppingCarId
                    };
                ...
                {
                    alert.error("Ocurrieron errores durante el pago");
                }
            }
        } catch (error) {
            document.querySelector('#pay_btn').disabled = false;
  ...
  return (
    <Fragment>
      <MetaData titulo={"Metodo De Pago"}/>
